    ...
    public BookUpdateEvent checkUpdateBook(IBook book) {
        for (Object bookJson : jsonArray) {
            if (bookJson instanceof JSONObject && ((String) ((JSONObject) bookJson).get("url")).startsWith(book.getUrl())) {
                if (MySql.stringToDate((String) ((JSONObject) bookJson).get("updated_at")).getTime() > book.getLastUpdate().getTime()
                        && (Long) ((JSONObject) bookJson).get("chr_length") - book.getSize() > Updater.minSizeUp) {
                    BookUpdateEvent bookUpdateEvent = Updater.subscribe.getBookEvent(book, Math.toIntExact((Long) ((JSONObject) bookJson).get("chr_length")) - book.getSize(), MySql.stringToDate((String) ((JSONObject) bookJson).get("updated_at")));
